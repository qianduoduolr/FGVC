...
class Summ_writer(object):
    def __init__(self, global_step=0, writer=None, log_freq=10, fps=8, scalar_freq=100, just_gif=False):
        self.writer = writer
    ...
    def sum_gt_predict_on_rgb(self, name, trajs, rgbs, trajs_g, valids=None, show_dots=True, frame_id=None, only_return=False, cmap='coolwarm', linewidth=1):


        rgb = self.summ_traj2ds_on_rgb(name, trajs, rgbs[:,-1], valids=None, show_dots=True, frame_id=None, only_return=True, cmap=cmap, linewidth=1)
        
        if rgb.dtype == torch.float32:
            rgb = back2color(rgb, False)
        
        rgb_first = back2color(rgbs[:,0], False)
        rgb_first = rgb_first[0].permute(1,2,0).cpu().numpy()
        rgb_first = cv2.cvtColor(rgb_first, cv2.COLOR_RGB2BGR)
        ...
        # for entire seq
        # B x T x 2
        for t in range(0, trajs_g.shape[1], 7):
                
            x = trajs_g[0,t,0,0]
            y = trajs_g[0,t,0,1]
            
            # if visibles[t] > 0:
            rgb = cv2.drawMarker(rgb, position=(int(x),int(y)),color=(255, 255, 0),markerSize = 6, markerType=cv2.MARKER_TILTED_CROSS, thickness=2)
                    
            if t == 0:
                rgb_first = cv2.drawMarker(rgb_first, position=(int(x),int(y)),color=(255, 255, 0),markerSize = 6, markerType=cv2.MARKER_TILTED_CROSS, thickness=2)
        ...
        cv2.imwrite(f'{name}_traj.png', rgb)
        cv2.imwrite(f'{name}_anno.png', rgb_first)
