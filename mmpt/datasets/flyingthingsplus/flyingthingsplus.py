...
class FlyingThingsDataset(Video_dataset_base):
    def __init__(self, dset='TRAIN', subset='all', use_augs=False, N=0, S=8, crop_size=(368, 496), valid_radius=1e9, version='ad', scale=8, sigma=-1, occ_version='al', force_twice_vis=True, drop=True, *args, **kwargs):
        ...
        for subset in subsets:
            rgb_root_path = os.path.join(self.root, "frames_cleanpass_webp", dset, subset)
            flow_root_path = os.path.join(self.root, "optical_flow", dset, subset)
            traj_root_path = os.path.join(self.root, "trajs_%s" % version, dset, subset)
            mask_root_path = os.path.join(self.root, "object_index", dset, subset)
            # heavy_root_path = os.path.join(self.root, "heavy_raft_flows_ab", dset, subset)

            folder_names = [folder.split('/')[-1] for folder in glob(os.path.join(traj_root_path, "*"))]
        ...
        print('found %d samples in %s (dset=%s, subset=%s, version=%s)' % (len(self.rgb_paths), self.root, dset, self.subset, version))
        ...
        for subset in subsets:
            ...
            flow_root_path = os.path.join(self.root, "optical_flow", dset, subset)
            mask_root_path = os.path.join(self.root, "object_index", dset, subset)
            occ_root_path = os.path.join(self.root, "occluders_%s" % occ_version, dset, subset)


            folder_names = [folder.split('/')[-1] for folder in glob(os.path.join(occ_root_path, "*"))]
            folder_names = sorted(folder_names)
        ...
        print('found %d occluders in %s (dset=%s, subset=%s, version=%s)' % (len(self.occ_rgb_paths), self.root, dset, self.subset, occ_version))
    ...
    def getitem_helper(self, index):
        ...
        img_names = [folder.split('/')[-1].split('.')[0] for folder in glob(os.path.join(cur_rgb_path, "*"))]
        ...
        if trajs.shape[1] <= self.N:
            if self.drop or trajs.shape[1] == 0:
                return None, False
            else:
                cur_n = trajs.shape[1]
                trajs = np.concatenate([trajs, np.zeros((self.S, self.N - cur_n, 2))], 1)
                visibles = np.concatenate([visibles, np.zeros((self.S, self.N - cur_n))], 1)
                valids = np.concatenate([valids, np.ones((self.S, self.N - cur_n))], 1)
        ...
        if self.clip_length < self.S:
            start = random.randint(0, self.S - self.clip_length - 1)
        else:
            start = 0
        
        sample = {
            'imgs': rgbs[start:start+self.clip_length],
            'occs': occs[start:start+self.clip_length],
            'masks': masks[start:start+self.clip_length],
            'trajs': trajs_full[start:start+self.clip_length],
            'visibles': visibles[start:start+self.clip_length],
            'valids': valids[start:start+self.clip_length],
             'modality': 'RGB',
            'num_clips': 1,
    ...
    def prepare_train_data(self, index):
        ...
        height, width = sample['imgs'][0].shape[:2]
    ...
    def add_occluders(self, rgbs, masks, trajs, visibles, valids):
        ...
        for oi in range(max_occ): # number of occluders:
            ...
            img_names = [folder.split('/')[-1].split('.')[0] for folder in glob(os.path.join(occ_rgb_path, "*"))]
            img_names = sorted(img_names)
