...
class FlyingThingsDatasetV2(Video_dataset_base):
    def __init__(self, dset='TRAIN', subset='all', use_spa_augs=False, use_augs=False, N=0, S=8, crop_size=(368, 496), valid_radius=1e9, version='ad', scale=8, sigma=-1, occ_version='al', force_twice_vis=True, drop=True, *args, **kwargs):
        ...
        self.use_spa_augs = use_spa_augs
    ...
    def getitem_helper(self, index):
        ...
        S_, N, D = trajs.shape
        start = random.randint(0, S_ - self.S)
        trajs = trajs[start:start+self.S]
        ...
        img_names = img_names[start_ind+start:start_ind+start+self.S]
    ...
    def add_occluders(self, rgbs, masks, trajs, visibles, valids):
        ...
        for oi in range(max_occ): # number of occluders:
            ...
            S_ = alt_trajs.shape[0]
            start = random.randint(0, S_ - self.S)
            alt_trajs = alt_trajs[start:start+self.S]
            occ_id = int(id_str)
            
            img_names = [folder.split('/')[-1].split('.')[0] for folder in glob(os.path.join(occ_rgb_path, "*"))]
            img_names = sorted(img_names)
            img_names = img_names[occ_start_ind+start:occ_start_ind+start+self.S]
