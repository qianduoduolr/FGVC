...
def mask_reduce_loss(loss, weight=None, reduction='mean', sample_wise=False):
    ...
    Note now support two types masking. ndim = 2 (mask entire sample) and ndim = 4 (mask some pixels of sample).    
    """
    ...
    # if reduction is mean, then compute mean over masked region
        ...
        if sample_wise:
            if loss.dim() > 2:
                weight = weight.sum(dim=[i for i in range(1, weight.dim())], keepdim=True)  # NCHW to N111
                loss = (loss / (weight + eps)).sum() / weight.size(0)
            else:
                loss = loss.mean(dim=-1).sum() / (weight.float().mean(dim=-1).sum())
