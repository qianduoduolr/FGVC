...
@OPERATORS.register_module()
class CorrLookupV2(nn.Module):
    """Heatmaps lookup operator.
    This operator is used in `RAFT<https://arxiv.org/pdf/2003.12039.pdf>`_
    Args:
        radius (int): the radius of the local neighborhood of the pixels.
            Default to 4.
    ...
    def forward(self, corr_pyramid: Sequence[Tensor], coords: Tensor):
        """Forward function of Correlation lookup.
        Args:
            corr_pyramid (Sequence[Tensor]): Correlation pyramid.
            flow (Tensor): Current estimated optical flow.
        ...
        _, _,  H, W = corr_pyramid[0].shape
        B, T, P, _ = coords.shape
    
        # for window
        dx = torch.linspace(
            -self.r, self.r, 2 * (self.r//self.interval) + 1, device=coords.device)
        dy = torch.linspace(
            -self.r, self.r, 2 * (self.r//self.interval) + 1, device=coords.device)
        delta = torch.stack(torch.meshgrid(dy, dx), axis=-1)
        delta_lvl = delta.view(1, 2 * self.r + 1, 2 * self.r + 1, 2)
        ...
        for i, corr in enumerate(corr_pyramid):
            ...
            centroid_lvl = coords.reshape(B * T * P, 1, 1, 2) / 2**i
            coords_lvl = centroid_lvl + delta_lvl

            corr = bilinear_sample(corr, coords_lvl, self.mode,
                                self.padding_mode, self.align_corners)
            corr = corr.view(B, T, P, -1)
            out_corr_pyramid.append(corr)

        corrs = torch.cat(out_corr_pyramid, -1)
...
class HeatLookup(nn.Module):
    ...
    def forward(self, heats: Sequence[Tensor], coords: Tensor):
        """Forward function of Correlation lookup.
        Args:
            corr_pyramid (Sequence[Tensor]): Correlation pyramid.
            flow (Tensor): Current estimated optical flow.
        ...
        # for window
        dx = torch.linspace(
            -self.r, self.r, 2 * (self.r//self.interval) + 1, device=heats.device)
        dy = torch.linspace(
            -self.r, self.r, 2 * (self.r//self.interval) + 1, device=heats.device)
        delta = torch.stack(torch.meshgrid(dy, dx), axis=-1)
        delta_lvl = delta.view(1, 2 * self.r + 1, 2 * self.r + 1, 2)

        centroid_lvl = grid.reshape(B * T * P, 1, 1, 2)
        coords_lvl = centroid_lvl + delta_lvl

        cur_heats = bilinear_sample(heats.reshape(B*T*P, 1, H, W), coords_lvl, self.mode,
                            self.padding_mode, self.align_corners)
        cur_heats = cur_heats.view(B, T, P, -1)
    
        return cur_heats
    
    
# @OPERATORS.register_module()
# class HeatLookupV2(nn.Module):
#     """Heatmaps lookup operator.
#     This operator is used in `RAFT<https://arxiv.org/pdf/2003.12039.pdf>`_
#     Args:
#         radius (int): the radius of the local neighborhood of the pixels.
...
#     def forward(self, heat_pyramid: Sequence[Tensor], coords: Tensor):
#         """Forward function of Correlation lookup.
#         Args:
#             corr_pyramid (Sequence[Tensor]): Correlation pyramid.
#             flow (Tensor): Current estimated optical flow.
...
#         out_heat_pyramid = []
#         for i, heat in enumerate(heat_pyramid):

#             centroid_lvl = coords.reshape(B * T * P, 1, 1, 2) / 2**i
#             coords_lvl = centroid_lvl + delta_lvl

#             heat = bilinear_sample(heat, coords_lvl, self.mode,
#                                 self.padding_mode, self.align_corners)
#             heat = heat.view(B, T, P, -1)
#             out_heat_pyramid.append(heat)

#         heats = torch.cat(out_heat_pyramid, -1)
